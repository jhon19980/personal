...
if (isset($_POST['login'])) {
	...
	$user = htmlspecialchars($user_unsafe, ENT_QUOTES, 'UTF-8');
	$pass = htmlspecialchars($pass_unsafe, ENT_QUOTES, 'UTF-8');

	// Consulta SQL utilizando PDO
	try {
		$query = $conexion->prepare("SELECT * FROM usuario WHERE  usuario=:user");
		...
		if ($counter > 0) {
			...
			if (password_verify($pass . $salt, $stored_password)) {

				// Registra el inicio de sesión en el historial
				$date = date("Y-m-d H:i:s");
				$remarks = "ha iniciado sesión en el sistema ";
				$history_query = $conexion->prepare("INSERT INTO history_log (user_id, action, date) VALUES (?, ?, ?)");
				$history_query->execute([$id, $remarks, $date]);
			}
		}
