...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $inactive_otrs = isset($_POST['otrs_activo']) ? 0 : 1;
    ...
    $sql_otrs = "UPDATE usuarioxpersonal SET  otrs_activo = ? WHERE id_usuario_personal = ?";
    ...
    $stmt_otrs = $conexion->prepare($sql_otrs);
    ...
    $result_otrs = $stmt_otrs->execute([$inactive_otrs, $id_usuario_personal]);
    ...
    if ($result_scse && $result_moodle && $result_correo && $result_otrs && $result_binaps && $result_unoe) {
        ...
        echo "Error al ejecutar la consulta: " . implode(" ", $stmt_scse->errorInfo()) . " " . implode(" ", $stmt_moodle->errorInfo())
            . " " . implode(" ", $stmt_correo->errorInfo()) . " " . implode(" ", $stmt_otrs->errorInfo()) . " " . implode(" ", $stmt_binaps->errorInfo())
